$(document).ready(function () {
    ...
    $(document).on('submit', '#prfl-img-form', function (e) {
        e.preventDefault();

        var formData = new FormData(this);
        formData.append('memberId', '사용자ID');

        $.ajax({
            url: '/mypageController/processUpdateProfileImage',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            success: function (response) {
                if (response.success) {
                    ...
                    // 필요한 추가 액션(예: 페이지 새로고침)
                } else {
                    alert('프로필 사진 변경 중 오류가 발생했습니다: ' + response.errors.join(', '));
                }
            },
