...
class SignupController extends CI_Controller {
    ...
    public function processMemberSignup() {
        ...
        if ($userName != NULL) {
            ...
            echo "<script>alert('이메일 입력값이 없습니다. 이메일을 입력해주세요'); history.back();</script>";
        }

        if ($password != NULL) {
            if (!preg_match('/^(?=.*[a-zA-Z])(?=.*\d)(?=.*[!@#$%^&*])[A-Za-z\d!@#$%^&*]{8,}$/', $password)) {
                echo "<script>alert('비밀번호는 영문, 숫자, 특수문자를 포함한 8자 이상이어야 합니다.'); history.back();</script>";
                return;
            }
        }
        ...
        if ($birth != NULL) {
            ...
            if ($birthYear > $currentYear || $birthYear < 1900) {
                echo "<script>alert('유효하지 않은 생년월일입니다.'); history.back();</script>";
                return;
            }
        }
        ...
        // 파일 업로드 처리
        $config['upload_path'] = '/assets/file/images/';
        ...
        if (!$this->upload->do_upload('file')) {
            ...
            $originalFileName = pathinfo($data['client_name'], PATHINFO_FILENAME);
            $fileExt = $data['file_ext'];
            $newFileName = $originalFileName . '-' . uniqid() . $fileExt;
            ...
            // 엔티티에 파일 경로 및 파일명 설정
            $member->setMemberFilePath($newFilePath);
